...
export function valideFormatLivres(){
    for (const cle in livres ){
        test(`Valide le format de la structure de livres ${cle}`, function() {
            let estPresent = true,
                clesAbsentes = [];
            ...
            affirme(estPresent, `L${nbCles > 1 ? 'es' : 'a'} clé${nbCles > 1 ? 's' : ''} ${clesAbsentes.join(', ')} de le livres ${cle} ${nbCles > 1 ? 'sont' : 'est'} absente${nbCles > 1 ? 's' : ''}`);
